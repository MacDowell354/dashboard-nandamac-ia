...
def _resolve_source() -> Tuple[Optional[pd.DataFrame], str]:
    if GOOGLE_SHEET_CSV_URL:
        try: return _fetch_google_csv(GOOGLE_SHEET_CSV_URL), "google-csv"
        except Exception as e: _log(f"ERRO CSV: {e}")
    if DATA_XLSX_PATH:
        try:
        ...
        except Exception as e: _log(f"ERRO XLSX: {e}")
...
def extract_projecao(df_raw: pd.DataFrame) -> pd.DataFrame:
    ...
    # remove linhas vazias e colunas 100% vazias
    ...
    # mantem principais colunas se existirem
...
def extract_profissoes_por_canal(df_raw: pd.DataFrame) -> pd.DataFrame:
    ...
    # heurística: cabeçalho está ~2 linhas abaixo (linhas coloridas na planilha)
    header_row = start + 2
    # localiza onde começa a primeira profissão (logo após header)
    data_start = header_row + 1

    # varre até 'Total Geral' ou linha toda vazia
    end = data_start
    while end < len(df_raw):
    ...
    # renomeia primeira coluna se estiver vazia
    if str(sub.columns[0]).strip() in ("", "nan", "None"):
        sub.columns = ["Profissao"] + list(sub.columns[1:])
    else:
        # normaliza nome
        sub.columns = ["Profissao"] + list(sub.columns[1:])
    ...
    sub = sub.fillna("")
    empty_cols = [c for c in sub.columns if c!="Profissao" and sub[c].astype(str).str.strip().eq("").all()]
    if empty_cols: sub = sub.drop(columns=empty_cols)

    # tenta converter números (remove % e vírgulas)
    for c in sub.columns:
        if c == "Profissao": continue
        serie = sub[c].astype(str).str.replace("%","",regex=False).str.replace(".","",regex=False).str.replace(",",".",regex=False)
        # se maioria é número, converte
        try:
...
def extract_estado_profissao(df_raw: pd.DataFrame) -> pd.DataFrame:
    ...
    while end < len(df_raw):
        first = str(df_raw.iloc[end, 0]).strip()
        if first == "" or first.lower().startswith("regiao por profissao"):
            break
        end += 1

    sub = df_raw.iloc[header_row:end].reset_index(drop=True)
    sub.columns = sub.iloc[0].tolist()
    sub = sub[1:].reset_index(drop=True).fillna("")

    # renomeia primeira coluna
    if str(sub.columns[0]).strip()=="":
        sub.columns = ["Estado"] + list(sub.columns[1:])
    else:
        sub.columns = ["Estado"] + list(sub.columns[1:])

    # converter números onde fizer sentido
    for c in sub.columns[1:]:
        serie = sub[c].astype(str).str.replace(".","",regex=False).str.replace(",",".",regex=False)
        num = pd.to_numeric(serie, errors="coerce")
        if num.notna().mean() >= 0.5:
            sub[c] = num.fillna(0)

    # filtra linhas realmente de estado (duas letras ou nome de UF válido)
    mask = sub["Estado"].astype(str).str.len().between(1, 30) & sub["Estado"].astype(str).str.isalpha()
...
def extract_regiao_profissao(df_raw: pd.DataFrame) -> pd.DataFrame:
    ...
    if str(sub.columns[0]).strip()=="":
        sub.columns = ["Regiao"] + list(sub.columns[1:])
    else:
        sub.columns = ["Regiao"] + list(sub.columns[1:])
...
def profissao_por_canal():
    ...
    # monta séries para um gráfico de barras por canal (top 8 somas)
    series = []
    if has_data:
        num_cols = [c for c in profcan.columns if c!="Profissao" and pd.api.types.is_numeric_dtype(profcan[c])]
        ...
        # prepara dados no formato [{canal, data:[{x:prof, y:valor}...]}]
...
def analise_regional():
    ...
    # séries para gráfico: total por Estado e por Região (somando todas as profissões)
    serie_uf, serie_reg = [], []
    if not uf.empty:
...
@app.get("/insights-ia")
def insights_ia():
    """Insights simples e automáticos a partir das seções parseadas."""
    df_raw = get_data()
    vendas = extract_vendas_realizadas(df_raw)
    profcan= extract_profissoes_por_canal(df_raw)
    uf     = extract_estado_profissao(df_raw)
    ...
    insights = []
    # 1) Crescimento de vendas por dia (se houver)
